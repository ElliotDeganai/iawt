...
export default {
    ...
    methods: {
        ...
        submit() {
            this.form.transform((data) => ({ ...data, _method: 'patch' })).post(route('profile.update'), { preserveScroll: true });
        },
    },
};
</script>
...
<template>
    <section>
        <header>
            ...
            <p class="mt-1 text-sm text-gray-600">Mettez à jour votre identité, vos coordonnées et votre photo de profil.</p>
        </header>
        ...
        <form class="mt-6 space-y-6" @submit.prevent="submit">
            ...
            <div class="flex items-center gap-4">
                <div class="h-16 w-16 shrink-0 overflow-hidden rounded-full bg-gray-100">
                    ...
                    <span v-else class="flex h-full w-full items-center justify-center text-xl text-gray-400">{{ form.first_name?.[0] }}{{ form.last_name?.[0] }}</span>
                </div>
                <div>
            ...
            <div v-if="mustVerifyEmail && !$page.props.auth.user.email_verified_at" class="text-sm">
                <p class="mt-2 text-gray-800">
                    ...
                    <Link :href="route('verification.send')" method="post" as="button" class="text-sm text-primary-600 underline hover:text-primary-700">Renvoyer l'e-mail de vérification.</Link>
                </p>
                <div v-show="status === 'verification-link-sent'" class="mt-2 text-sm font-medium text-green-600">Un nouveau lien de vérification a été envoyé.</div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <TextInput id="phone" v-model="form.phone" type="tel" class="mt-1 block w-full" placeholder="+225..." />
                    <InputError class="mt-2" :message="form.errors.phone" />
                </div>
                <div>
                    <InputLabel for="whatsapp" value="WhatsApp" />
                    <TextInput id="whatsapp" v-model="form.whatsapp" type="tel" class="mt-1 block w-full" placeholder="+225..." />
