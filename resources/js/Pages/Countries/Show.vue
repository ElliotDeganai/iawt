...
export default {
    ...
    data() { return { lightbox: null, mapError: false }; },
    computed: {
        bonASavoir() {
            return [
        ...
        placesEntries() {
            ...
            for (const p of (this.country.country_places || [])) {
                ...
                if (!cat) continue;
                if (!groups[cat.id]) {
                    groups[cat.id] = { key: cat.slug, label: cat.name, iconPath: PLACE_ICONS[cat.icon] || PLACE_ICONS[cat.slug] || PLACE_ICONS.building, items: [] };
                }
        ...
        isAutoMap() {
            return !this.country.map_image && !!this.country.flag_code;
        },
    ...
    beforeUnmount() { window.removeEventListener('keydown', this._keyHandler); },
    methods: {
        formatMonth(date) { return new Date(date).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' }); },
        infoIcon(key) { return INFO_ICONS[key] || ''; },
        embedUrl(url) {
        ...
        openLightbox(index) { this.lightbox = { index, photos: this.galleryPhotos }; document.body.style.overflow = 'hidden'; },
        closeLightbox() { this.lightbox = null; document.body.style.overflow = ''; },
        lightboxNext() { if (this.lightbox) this.lightbox.index = (this.lightbox.index + 1) % this.lightbox.photos.length; },
        lightboxPrev() { if (this.lightbox) this.lightbox.index = (this.lightbox.index - 1 + this.lightbox.photos.length) % this.lightbox.photos.length; },
    },
};
</script>
...
<template>
    ...
    <PublicLayout>
        <section class="bg-primary-800 px-4 py-10 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-5xl">
                <Link :href="route('countries.index')" class="mb-6 inline-flex items-center gap-1.5 text-xs text-primary-200 hover:text-white">
                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>
                    Archives des pays à l'honneur
                </Link>
                <div class="flex flex-col-reverse lg:flex-row lg:items-stretch gap-6 lg:gap-8">
                    <div class="flex-1 min-w-0">
                        ...
                        <p v-if="country.content" class="mb-6 text-sm leading-relaxed text-primary-100 opacity-90">{{ country.content }}</p>
                        <div v-if="bonASavoir.length" class="grid grid-cols-2 gap-2">
                            <div v-for="f in bonASavoir" :key="f.label" :class="f.iconKey === 'transport' ? 'col-span-2' : ''" class="rounded-lg border border-white/10 bg-white/7 px-3 py-2.5">
                    ...
                    <div v-if="mapSrc && !mapError" class="shrink-0 w-full lg:w-1/2 lg:self-stretch">
                        <img :src="mapSrc" class="h-full w-full object-scale-down" :style="isAutoMap ? 'filter: brightness(0) invert(1); opacity: 0.55;' : ''" alt="Carte du pays" @error="mapError = true" />
                    </div>
                </div>
            </div>
        </section>
        ...
        <div class="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 space-y-12">
            <section v-if="country.highlights?.length">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div v-for="(h, i) in country.highlights" :key="i" class="flex items-start gap-3 rounded-xl border border-gray-100 bg-white p-4">
                        <span class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary-600 text-[10px] font-semibold text-white">{{ i + 1 }}</span>
                        <div><p class="text-sm font-medium text-gray-900">{{ h.name }}</p><p v-if="h.description" class="mt-1 text-xs leading-relaxed text-gray-500">{{ h.description }}</p></div>
                    </div>
                </div>
            </section>
            ...
            <section v-if="country.tourism_agencies?.length">
                ...
                <div class="divide-y divide-gray-100 overflow-hidden rounded-xl border border-gray-100 bg-white">
                    <div v-for="(a, i) in country.tourism_agencies" :key="i" class="px-5 py-3.5">
                        ...
                        <p v-if="a.description" class="mt-1 text-xs leading-relaxed text-gray-500">{{ a.description }}</p>
                    </div>
                </div>
            </section>

            <section v-if="country.responsible_tourism?.length">
                <div class="mb-3 flex items-center gap-2.5">
                    ...
                    <p class="text-[10px] font-medium uppercase tracking-widest text-gold-600">Et si on voyageait autrement ?</p>
                </div>
                <div class="divide-y divide-gray-100 overflow-hidden rounded-xl border border-gray-100 bg-white">
                    <div v-for="(r, i) in country.responsible_tourism" :key="i" class="px-5 py-3.5">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <p class="text-sm font-medium text-gray-900">{{ r.name }}</p>
                            <div class="flex flex-wrap gap-4 text-xs">
                                <a v-if="r.website" :href="r.website" target="_blank" rel="noopener" class="text-primary-600 hover:underline">Site web →</a>
                                <a v-if="r.email" :href="`mailto:${r.email}`" class="text-primary-600 hover:underline">{{ r.email }}</a>
                                <span v-if="r.phone" class="text-gray-500">{{ r.phone }}</span>
                            </div>
                        </div>
                        <p v-if="r.description" class="mt-1 text-xs leading-relaxed text-gray-500">{{ r.description }}</p>
                    </div>
                </div>
            </section>
            ...
            <section v-for="section in placesEntries" :key="section.key">
                <div class="mb-3 flex items-center gap-2.5">
                    <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary-600 text-white"><svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path :d="section.iconPath" /></svg></span>
                    <p class="text-[10px] font-medium uppercase tracking-widest text-gold-600">{{ section.label }}</p>
                </div>
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div v-for="(p, i) in section.items" :key="i" class="rounded-xl border border-gray-100 bg-white p-4">
                        ...
                        <a v-if="p.link" :href="p.link" target="_blank" rel="noopener" class="mt-2 inline-flex items-center gap-1 text-xs font-medium text-primary-600 hover:underline">En savoir plus →</a>
                    </div>
                </div>
            </section>
            ...
            <section v-if="galleryPhotos.length || galleryVideos.length">
                ...
                <div v-if="galleryPhotos.length" class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                    <button v-for="(photo, index) in galleryPhotos" :key="photo.id" type="button" class="group relative overflow-hidden rounded-xl bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500" @click="openLightbox(index)">
                        <img :src="photo.url" class="aspect-square w-full object-cover transition-transform duration-300 group-hover:scale-105" :alt="photo.caption || ''" />
                        <div class="absolute inset-0 flex items-center justify-center bg-black/0 transition-colors duration-200 group-hover:bg-black/20">
                            <svg class="h-8 w-8 text-white opacity-0 drop-shadow transition-opacity duration-200 group-hover:opacity-100" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35M11 8v6M8 11h6"/></svg>
                        </div>
                        <div v-if="photo.caption" class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 px-3 py-2 text-left"><p class="text-xs text-white">{{ photo.caption }}</p></div>
                    </button>
                </div>
                <div v-if="galleryVideos.length" class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div v-for="video in galleryVideos" :key="video.id" class="overflow-hidden rounded-xl bg-gray-900">
                        <div class="relative aspect-video"><iframe v-if="video.url" :src="embedUrl(video.url)" class="absolute inset-0 h-full w-full" frameborder="0" allowfullscreen></iframe></div>
        ...
        <Teleport to="body">
            <div v-if="lightbox && lightboxPhoto" class="fixed inset-0 z-50 flex items-center justify-center bg-black/90" @click.self="closeLightbox">
                <button type="button" class="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20" @click="closeLightbox"><svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"/></svg></button>
                <button v-if="lightbox.photos.length > 1" type="button" class="absolute left-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20" @click="lightboxPrev"><svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7"/></svg></button>
                <div class="mx-20 flex max-h-[90vh] max-w-5xl flex-col items-center">
                    <img :key="lightboxPhoto.id" :src="lightboxPhoto.url" class="max-h-[80vh] max-w-full rounded-lg object-contain shadow-2xl" :alt="lightboxPhoto.caption || ''" />
                    <div v-if="lightboxPhoto.caption" class="mt-3 text-sm text-white/70">{{ lightboxPhoto.caption }}</div>
                    <div v-if="lightbox.photos.length > 1" class="mt-2 text-xs text-white/40">{{ lightbox.index + 1 }} / {{ lightbox.photos.length }}</div>
                </div>
                <button v-if="lightbox.photos.length > 1" type="button" class="absolute right-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20" @click="lightboxNext"><svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"/></svg></button>
            </div>
        </Teleport>
    </PublicLayout>
</template>