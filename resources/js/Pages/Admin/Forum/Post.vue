...
export default {
    ...
    data() {
        return { form: useForm({ body: '', attachments: [] }) };
    },
    methods: {
        submitReply() {
...
<template>
    ...
    <PublicLayout>
        ...
        <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm mb-6">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-3 text-sm text-gray-700 leading-relaxed whitespace-pre-line">{{ post.body }}</div>
            ...
            <!-- Replies -->
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">{{ post.replies?.length || 0 }} réponse{{ (post.replies?.length || 0) > 1 ? 's' : '' }}</p>
            <div class="space-y-3 mb-6">
                <div v-for="r in post.replies" :key="r.id" class="rounded-xl border border-gray-100 bg-white px-5 py-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="mt-2 text-sm text-gray-600 whitespace-pre-line">{{ r.body }}</div>
            ...
            <form v-if="$page.props.auth?.user" class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm" @submit.prevent="submitReply">
                <textarea v-model="form.body" rows="3" class="w-full rounded-lg border-gray-200 text-sm" placeholder="Votre réponse…"></textarea>
                <div class="mt-3 flex items-center gap-3">
                    <button type="submit" :disabled="form.processing || !form.body" class="rounded-full bg-primary-600 px-5 py-2 text-sm font-semibold text-white hover:bg-primary-700 disabled:opacity-50">Répondre</button>
