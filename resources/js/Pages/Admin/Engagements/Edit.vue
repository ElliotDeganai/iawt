...
export default {
    ...
    data() {
        return {
            icons: ICONS,
            form: useForm({
                icon: this.engagement.icon,
                title: this.engagement.title,
                description: this.engagement.description,
                position: this.engagement.position,
            }),
        };
    },
    methods: {
        submit() {
            this.form.put(route('admin.engagements.update', this.engagement.id));
        },
    },
};
</script>
...
<template>
    <Head title="Administration — Modifier l'engagement" />

    <AdminLayout>
        <template #header>
            <h1 class="text-lg font-semibold text-gray-800">Modifier « {{ engagement.title }} »</h1>
        </template>
